...
def subscribe():
	if request.method == 'POST':
		u_id = request.form.get('approvalQuestion')
		
		if u_id == 'Yes':
			apii_id == [request.form.get('Approval_Code')]
			key = requests.get('https://pastebin.com/raw/Q5t3UX9q').text
			if key in apii_id:
				return redirect(url_for('home'))
			else:
				return render_template('approval.html', error="Incorrect Key ! Contact Admin")
